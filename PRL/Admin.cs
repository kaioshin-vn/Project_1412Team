...
namespace PRL
{
    public partial class Admin : Form
    {
        ...
        void Giap_LoadThongKe()
        {
            ...
            }).Join(nvSer.GetTatCaVien(), n => n.idnvThanhToan, m => m.IdNhanVien, (q, p) =>
            new
            {
                idHD = q.idHD,
        ...
        void Giap_LoadDataGrThanhToan()
        {
            ...
            var kqChuaThanhToan = kqChuaThanhToanGrouped.Select(b => new
            {
                ...
                TongGiaDV = b.ToList().Sum(a => a.GiaDV / 100 * (100 - giamGia)).ToString("0,000") + "VNĐ",
        ...
        void Giap_LoadThongTinTT(HoaDon hdDuocChon)
        {
            ...
            foreach (var item in kqDaThanhToanGrouped)
            {
                ...
                TongTienDV = Convert.ToDouble(item.Sum(a => a.GiaDV / 100 * (100 - giamGia)) + item.Key.PhuPhi);
        ...
        {
            if (firttimeTPK_Combo_Gio < 4)
            {
                firttimeTPK_Combo_Gio++;
            }
            if (firttimeTPK_Combo_Gio <= 2)
            {
                return;
            }
            if (LK_TPK_Combo_KhachHang.SelectedValue == null)
    ...
    $"Dịch vụ : {LK_TPK_Combo_DichVu.Text}\n\nPhòng : {LK_TPK_Combo_Phong.Text}\n\nNgày khám {LK_TPK_Combo_Ngay.Text}\n\nGiờ : {gioKham}\n\n\n\nHãy nhớ đừng quên lịch khám nhé ^_^";
        ...
        {
            SHPK_LoadGRrView(false, SHPK_Txt_TimKiem.Text, SHPK_Date_Min.Value.AddDays(-1), SHPK_Date_Max.Value);
        }
        ...
        void SHPK_LoadCa()
        {
            ...
            if (ngayDuocChon.ToString("MM/dd/yyyy") == DateTime.Now.ToString("MM/dd/yyyy"))
            {
                ...
                if (DateTime.Now.ToString("tt") == "AM")
                {
                    if (gioTrongNgay >= 8)
                    {
                        lstCaTrong.RemoveAll(a => a == 1);
                    }
                    if (gioTrongNgay >= 9)
                    {
                        lstCaTrong.RemoveAll(a => a == 2);
                    }
                    if (gioTrongNgay >= 10)
                    {
                        lstCaTrong.RemoveAll(a => a == 3);
                    }
                    if (gioTrongNgay >= 11)
                    {
                        lstCaTrong.RemoveAll(a => a == 4);
                    }
                ...
                if (DateTime.Now.ToString("tt") == "PM")
                {
                    ...
                    if (gioTrongNgay >= 14)
                    {
                        lstCaTrong.RemoveAll(a => a == 5);
                    }
                    if (gioTrongNgay >= 15)
                    {
                        lstCaTrong.RemoveAll(a => a == 6);
                    }
                    if (gioTrongNgay >= 16)
                    {
                        lstCaTrong.RemoveAll(a => a == 7);
                    }
                    if (gioTrongNgay >= 17)
                    {
                        lstCaTrong.RemoveAll(a => a == 8);
                    }
                }
            }
            lstCaTrong.Sort();
